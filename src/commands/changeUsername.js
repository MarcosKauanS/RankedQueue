...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('trocar') // Comando principal
    .setDescription('Permite que o usuário troque seu nickname')
    .addSubcommand(sub =>
      sub
        .setName('nick') // Subcomando
        .setDescription('Trocar o nickname')
  ...
  async execute(interaction, db) {
    // Garante que o subcomando seja "nick"
    if (interaction.options.getSubcommand() !== 'nick') return;
    ...
    try {
      ...
      await interaction.reply({
        content: `✅ Seu nickname foi atualizado para: **${finalNick}**`,
        ephemeral: true
      });
    } catch (err) {
      console.error('Erro ao trocar nickname:', err);
      await interaction.reply({
        content: '❌ Não foi possível trocar seu nickname.',
        ephemeral: true
      });
    }
  }
